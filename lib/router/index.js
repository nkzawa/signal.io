...
Router.prototype._dispatch = function(socket, next) {
  ...
  (function pass(i, err) {
    ...
    function nextRoute(err) {
      pass(socket._route_index + 1, err);
    }

    // match route
    socket.route = route = self.matchRequest(socket, i);
...
Router.prototype.matchRequest = function(socket, i, head) {
...
Router.prototype.match = function(name, i, head) {
  var socket = { nsp: { name: name} };
  return this.matchRequest(socket, i, head);
};
